---- MODULE RecallAdvancement_TTrace_1772711451 ----
EXTENDS RecallAdvancement, Sequences, TLCExt, RecallAdvancement_TEConstants, Toolbox, Naturals, TLC
...
_prop ==
    ~(([]<>(
            ...
            /\
            recordLinkMode = ("voice_session_id")
            /\
...
\* to `JsonSerialize`. For example, a sub-sequence of _TETrace.
    \* ASSUME
    \*     LET J == INSTANCE Json
    \*         IN J!JsonSerialize("RecallAdvancement_TTrace_1772711451.json", _TETrace)


_view ==
    <<recordLinkMode, storyRecordFound, pc, sessionSource, currentIndex, slotsComplete, advanceInFlight, IF TLCGet("level") = _TTraceLassoEnd + 1 THEN _TTraceLassoStart ELSE TLCGet("level")>>
=============================================================================
...
\*EXTENDS RecallAdvancement, IOUtils, RecallAdvancement_TEConstants, TLC
\*
\*trace == IODeserialize("RecallAdvancement_TTrace_1772711451.bin", TRUE)
\*
\*=============================================================================
\*

---- MODULE RecallAdvancement_TETrace ----
EXTENDS RecallAdvancement, RecallAdvancement_TEConstants, TLC

trace == 
    <<
    ([sessionSource |-> "session",pc |-> "idle",slotsComplete |-> TRUE,storyRecordFound |-> FALSE,recordLinkMode |-> "voice_session_id",advanceInFlight |-> FALSE,currentIndex |-> 0]),
    ([sessionSource |-> "session",pc |-> "move_on_detected",slotsComplete |-> TRUE,storyRecordFound |-> FALSE,recordLinkMode |-> "voice_session_id",advanceInFlight |-> FALSE,currentIndex |-> 0]),
    ([sessionSource |-> "session",pc |-> "checking_progress",slotsComplete |-> TRUE,storyRecordFound |-> FALSE,recordLinkMode |-> "voice_session_id",advanceInFlight |-> FALSE,currentIndex |-> 0]),
    ([sessionSource |-> "session",pc |-> "blocked",slotsComplete |-> TRUE,storyRecordFound |-> FALSE,recordLinkMode |-> "voice_session_id",advanceInFlight |-> FALSE,currentIndex |-> 0]),
    ([sessionSource |-> "session",pc |-> "idle",slotsComplete |-> TRUE,storyRecordFound |-> FALSE,recordLinkMode |-> "voice_session_id",advanceInFlight |-> FALSE,currentIndex |-> 0])
    >>
----
...
---- MODULE RecallAdvancement_TEConstants ----
EXTENDS RecallAdvancement

CONSTANTS _TTraceLassoStart, _TTraceLassoEnd

=============================================================================

---- CONFIG RecallAdvancement_TTrace_1772711451 ----
CONSTANTS
    NUM_QUESTIONS = 4
_TTraceLassoStart = 2
_TTraceLassoEnd = 5
...
ALIAS
    _expression
=============================================================================
\* Generated on Thu Mar 05 06:50:51 EST 2026